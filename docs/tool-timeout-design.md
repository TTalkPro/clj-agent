...
> ### ⚠️ 终态修订（2026-07-16 用户拍板）：缺省不超时，且缺省来源是**引擎**而非 filter
>
> 本章成文时，「整体缺省」的载体是 `advisor/timeout-filter`（§3.3 的 P1）。**已变**：
>
>     工具声明 deftool {:timeout ms}
>       > 引擎缺省 (…-tool-calling-manager {:timeout ms})
>       > **不超时**
>
> **两条改动**：
> 1. **缺省不超时**——框架不替调用方决定何时放弃。都没给就不起线程、零开销。
>    （与 beamai 同日的动作一致：它把三层缺省全改成了 `infinity`，见 §5.5。）
> 2. **缺省超时属于引擎，不属于 filter**——manager 就是执行引擎（线程模型 + 隔离
>    边界 + 调度策略），「这批工具最多跑多久」是执行策略的一部分。这正是 beamai
>    `manager_opts.tool_timeout` 的立场：*「manager 是执行引擎，时间上限属于执行策略，
...
> `:timeout` 接管，剩下的唯一辩护「按工具名/标签动态决定时限」是假想需求——
> 用户真要时自己写 5 行 `:tool` filter 包 `tool/call-with-timeout` 即等价。
> 三层优先级也是复杂度税（beamai 同样只有两个来源：tool_spec + manager opts）。
> `build-func-def` 的 `:timeout` 字段随之删除（唯一读者就是该 filter——
> 没有读者的字段就是下一个死选项）。已入 check_docs 墓碑，防文档复活。
> 机制本体 `tool/call-with-timeout` 保留（`invoke-tool` 消费）。
>
> 下文 §3.1–§3.4 的分析与否决（层 2 / 层 3、方案 B）**全部不受影响**：它们讲的是
> 「要不要复刻 beamai 的层数」，与缺省值放哪儿正交。文中其余提及
> `timeout-filter` 的段落均为历史记录（它曾是 P1 的载体与两个真 bug 的现场），
> 按「设计文档是历史记录」的约定保留原文。
...
1. **层 3 的主要理由是「防止工具带崩调用者」**，而这在 JVM 根本不存在。beamai 的原文：
   ...
   **JVM 没有 link，没有退出信号传播。** 子线程抛异常打不死父线程。
   `invoke-one` 的 try/catch 就是本批的错误边界。**层 3 在防一个我们没有的问题。**

   > **⚠️ 修订（2026-07-16 code review #5）**：本条原文写「`invoke-one` 的 try/catch
   > **已经**是完备的错误边界」——**当时并不完备**：它只 `catch Exception`，于是
...
> ### ⚠️ 已修订（2026-07-16 code review）：方案 C 的否决只对了一半
>
> 上面这条把两件事捆在了一起否掉：**位置**（下沉到调用原语）与**恒定**（每个调用
> 无条件起线程）。**该否的只有后者。**
>
...
>   批次**策略（重试几次、能否并发、要不要回灌），属 react；而 `:timeout` 界定的是
...
>
> 另外，方案 C 否决理由里的「换来一个**打不断的**超时」这句本身也已被 §2.2 修订块
> 推翻（虚拟线程上阻塞 IO 真能取消）。**一条否决记录的两条论据都倒了，剩下的那条
> （不要无条件加线程）仍然成立，且已单独兑现**——这正是 `design-principles.md`
> §1.3「已建的抽象，论据倒了一条 → 重新称剩下的论据有多重」的一次实践。
...
## 5.7 后记：声明查询合成一张表 + 同名装配期拒绝（2026-08-25）

§1.2 那个「`deftool :timeout` 是死选项」的真 bug，根因在本文里已经写过：
`:serial` / `:retry` / `:return-direct` 各有 inline 分支，**独 `:timeout` 漏了**。
...
四遍。抄第四遍时漏一支，是这种结构的必然而非偶然。

现在两个来源在**装配期**汇成一张 `ToolMeta` 表（`:func-def` / `:serial` /
`:retry` / `:timeout` / `:return-direct`），四个查询退化成一次 map 查找 + 一次
字段读，签名不变。`:retry` 的默认值 merge 也从每次查询挪到装配期。新增字段
只需在建表的两个构造函数里各加一行，不再有「第五处忘了改」的位置。

**合表顺带暴露了一个更深的不一致**：那四处双分支是 **var 优先**，而
`invoke-tool` 的执行分派一直是 **内联优先**——同名时会「按 var 的超时/重试策略
...
只留得下一个，「模型看到的」与「实际执行的」就此对不上，且**没有任何运行期症状
可查**（工具照常执行，只是执行的可能不是模型以为的那个）。「选一个赢家」是在给
配置错误编造语义。要替换某个工具，调用方该在传 `:tools` 之前处理自己的列表。

校验顺序也是有讲究的：重名校验排在 `validate-tool-timeouts!` /
`validate-tool-retries!` **之前**——重名时 `var-map` / `inline-handler-map` 的
`into` 已经把重复悄悄吃掉了，此后校验的是「幸存者」，等于在错误的地基上校验。
有测试钉住这个顺序（两个同名内联工具且都带非法 `:timeout`，断言报的是重名）。

> **本文其余部分的 `:timeout` 语义一字未变**：优先级链（工具声明 > 引擎缺省 >
> 不超时）、放弃等待≠终止执行、只包 terminal 不包 filter 链，全部照旧。改的只是
> 「这些声明存在哪、怎么查」。
