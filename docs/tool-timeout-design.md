...
- **CPU 密集循环** —— 正则回溯、大 JSON 解析、死循环
- **`InputStream.read()` 读普通 socket** —— **绝大多数 HTTP 客户端就长这样**。
  这条最要命：「调用外部 API 的工具」正是最需要超时的那类，也正是打不断的那类。
...
**测试（+7 tests / +24 assertions，全套 307/1251/0）**，逐条对应 §3/§2 的论点：
