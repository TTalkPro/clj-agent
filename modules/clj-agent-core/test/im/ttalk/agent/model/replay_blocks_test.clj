(ns im.ttalk.agent.model.replay-blocks-test
  "P3 回传契约的 core 侧：chat-model 归一化时**探测**可选协议，把不透明载荷搬进响应。
...
(deftest chat-model-threads-replay-blocks
  (testing "实现了协议 → 归一化响应带上载荷"
    (let [cm (chat-model/create-chat-model (->ReplayingProvider) {:model "m"})
          r (chat-model/call cm (req/chat-request [{:role :user :content "hi"}]))]
  ...
  (testing "没实现 → nil，其余一切照旧（老 provider 零改动）"
    (let [cm (chat-model/create-chat-model (->PlainProvider) {:model "m"})
          r (chat-model/call cm (req/chat-request [{:role :user :content "hi"}]))]
  ...
  (testing "流式路径同样带上载荷（流式响应的块形状与非流式同构，实测确认）"
    (let [cm (chat-model/create-chat-model (->ReplayingProvider) {:model "m"})
          r (chat-model/stream-call cm (req/chat-request [{:role :user :content "hi"}]) (fn [_] nil))]
      (is (= (:content raw) (:data (resp/response-replay-blocks r)))))))
