...
## P11 — Provider 变体差异（MiniMax thinking，2026-07-28）✅ 同日完成
...
- [x] **P3 暂不立项**（回传契约 + 新协议 + 中立 `:blocks` 字段）——是**证据不足**，
  不是否决：M3 上的行为差异是真的，但「模型少想几次」与「答得更差」之间没有桥。
  §1 判据要的是「不建就做不到的能力」，而成本不对称（建的代价永久、不建的代价
  目前为零）。设计文档 §7.5 **预先定死**了下一个实验与判据（答案唯一可判定、
  难到必须逐轮思考的任务，A/B 各 ≥20 次；正确率显著低 → 立项，无差别 → 永久不立项），
  免得看到数据再找理由。
